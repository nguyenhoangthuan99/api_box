...
async def decodes(CameraID: str, fileb: bytes = File(...),):
    ...
    for i in range(np.array(bbox).shape[0]):
      #  print(bbox[i].shape)
        ...
        bbox[i][2] = bbox[i][3]
        bbox[i][3] = tmp
        cv2.polylines(img, [bbox[i].astype(np.int32).reshape(
    ...
    for i in range(np.array(bbox).shape[0]):
        ...
        final[bb_name] = str([bbox[i][0][0], bbox[i][0][1], bbox[i][1][0], bbox[i]
                              [1][1], bbox[i][2][0], bbox[i][2][1], bbox[i][3][0], bbox[i][3][1]])
    ...
    plate_res = []
    if len(bbox) == 1:
        final["plate_box1"] = str([bbox[0][0][0], bbox[0][0][1], bbox[0][1][0],
                                   bbox[0][1][1], bbox[0][2][0], bbox[0][2][1], bbox[0][3][0], bbox[0][3][1]])
        ...
        for i in range(np.array(bbox).shape[0]):
            for j in range(np.array(bbox).shape[0]):
                if i != j:
                    x1, y1, x2, y2, x3, y3, x4, y4 = bbox[i][0][0], bbox[i][0][1], bbox[i][1][
                        0], bbox[i][1][1], bbox[i][2][0], bbox[i][2][1], bbox[i][3][0], bbox[i][3][1]
                    x10, y10, x20, y20, x30, y30, x40, y40 = bbox[j][0][0], bbox[j][0][1], bbox[j][
                        1][0], bbox[j][1][1], bbox[j][2][0], bbox[j][2][1], bbox[j][3][0], bbox[j][3][1]
                    X0 = (x1+x2+x3+x4)/4.
                    Y0 = (y1+y2+y3+y4)/4.
    ...
    for i in range(len(plate_res)):
        ...
        final[key_name] = str([less[0][0], less[0][1], less[1][0], less[1]
                               [1], greater[2][0], greater[2][1], greater[3][0], greater[3][1]])
    ...
    for i in range(np.array(bbox).shape[0]):
        if i not in done:
            ...
            key_name = "plate_box" + str(idx)
            key_txt = "plate_text"+str(idx)
            final[key_name] = str([bbox[i][0][0], bbox[i][0][1], bbox[i][1][0], bbox[i]
                                   [1][1], bbox[i][2][0], bbox[i][2][1], bbox[i][3][0], bbox[i][3][1]])
...
async def changePassword(username: str, old_password: str, new_password: str, current_user: User = Depends(get_current_active_user)):
    if current_user.username != username or not verify_password(old_password, current_user.hashed_password):
        return {"result": "Fail", "message": "user name or old password wrong"}, 200
    else:
        fake_users_db[current_user.username]["hashed_password"] = get_password_hash(
            new_password)

        with open('login.pickle', 'wb') as handle:
            pickle.dump(fake_users_db, handle,
                        protocol=pickle.HIGHEST_PROTOCOL)
        return {"result": "Success", "message": ""}, 200


@app.get("/getCameras")
def getCameras(current_user: User = Depends(get_current_active_user)):
    return {"result": list_camera}, 200


@app.post("/addCamera")
def addCamera(CameraID: str, password: str, current_user: User = Depends(get_current_active_user)):
    global list_camera

    if not verify_password(password, current_user.hashed_password):
        return {"result": "Fail", "message": "Password wrong! Please enter right password to add camera"}, 200

    if CameraID in list_camera:
        return {"result": "Fail", "message": CameraID + " already exist in database"}, 200
    else:
        list_camera.append(CameraID)
        with open("list_cam.txt", "w") as writer:
            for i in list_camera:
                writer.write(i+"\n")
        return {"result": "Success", "message": "Successfully add "+CameraID + " to database"}, 200


@app.post("/deleteCamera")
def deleteCamera(CameraID: str, password: str, current_user: User = Depends(get_current_active_user)):
    global list_camera

    if not verify_password(password, current_user.hashed_password):
        return {"result": "Fail", "message": "Password wrong! Please enter right password to add camera"}, 200

    if CameraID not in list_camera:
        return {"result": "Fail", "message": CameraID + " does not exist in database"}, 200
    else:
        list_camera.remove(CameraID)
        with open("list_cam.txt", "w") as writer:
            for i in list_camera:
                writer.write(i+"\n")
        return {"result": "Success", "message": "Successfully delete "+CameraID + " from database"}, 200
...
async def checkUpdate(username: str, password: str, current_user: User = Depends(get_current_active_user)):
    if current_user.username != username or not verify_password(password, current_user.hashed_password):
        ...
        with eventlet.Timeout(10):
            x = requests.post(url="http://14.177.239.164:9090/checkUpdate?username=" +
